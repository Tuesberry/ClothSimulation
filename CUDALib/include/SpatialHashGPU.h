...
{
public:
	SpatialHashGPU(float particleDiameter, int maxNumObjects, VtSimParams simParams)
	{
		this->simParams = simParams;
		// BUG_LOG: m_spacing was miswritten as int
		m_spacing = particleDiameter * simParams.hashCellSizeScalar;
		m_tableSize = 2 * maxNumObjects;

		neighbors.resize(maxNumObjects * simParams.maxNumNeighbors);
	...
	void SetInitialPositions(const VtBuffer<glm::vec3>& positions)
	{
	...
	void Hash(const VtBuffer<glm::vec3>& positions)
	{
		...
		params.maxNumNeighbors = simParams.maxNumNeighbors;
		params.particleDiameter2 = simParams.particleDiameter * simParams.particleDiameter;
...
private:
	...
	void Test(const VtBuffer<glm::vec3>& positions)
	{
		...
		for (int i = 0; i < positions.size(); i++)
		{
			...
			for (int k = 0; k < 64; k++)
			{
				uint neighbor = neighbors[i * simParams.maxNumNeighbors + k];
